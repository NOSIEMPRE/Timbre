...
async def _handler(url: str) -> dict:
    try:
        ...
        return {"content": "", "error": "playwright not installed", "url": url}
...
browse_url = Tool(
    name="browse_url",
    description=(
        "Fetch the full text content of a URL using a headless browser. "
        "Use this to retrieve complete articles, especially from paywalled sources "
        "when the user has configured their browser cookies."
    ),
    parameters={
        "type": "object",
        "properties": {
