...
**vc-sourcing** 只需要一个赛道关键词，不用提前知道公司名。并行扫描 HackerNews Show HN、YC W25/S25 批次、ProductHunt 新品、TechCrunch 早期报道、GitHub trending，输出按 vc_appeal 排序的项目列表。

**founder-research** 输入创始人名字或公司名，经过实体识别、并行搜索、综合分析，生成8节格式的尽调档案，每条事实标来源编号，风险点按 P0/P1/P2 分级，支持 Playwright 访问付费内容。保存时同步更新 index.md、investor 实体页和赛道页。

**Stage 2.5 创始人补全** 处理 sourcing 结果里的 `founder: unknown`。自动对 LinkedIn、Crunchbase、TechCrunch 发起定向搜索，再过多层名称过滤。实测识别率从约 40% 提升到了 78%。

---

## Obsidian 知识图谱

`founder-research` 每次运行，除了写创始人档案，还会更新整个知识图谱：
