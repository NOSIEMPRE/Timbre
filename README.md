...
Timbre 做的就是这件事。给它一个输入——创始人姓名、公司名或核心产品名——它会自己找到对的人，从四个维度展开调研，输出一份有判断力的情报档案，而不只是数据的堆砌。
...
Timbre 只能访问公开信息。薪酬结构、真实股权比例、内部管理矛盾不在覆盖范围内。

LinkedIn 在中国的渗透率偏低，部分国内团队的组织信息会不完整。融资数据通常比实际晚披露 3 到 6 个月。

访问付费媒体需要配置 cookie 文件，否则只能从搜索摘要中获取信息。

产品名与公司名不一致时，建议同时附上创始人姓名，Stage 1 的识别准确率会更高。
